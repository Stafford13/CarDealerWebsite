...
namespace DateTimeDemo.UI
{
    public class UserIO : IUserIO
    {
        ...
        public  Character PromptUserForNewCharacter()
        {
            ...
            decimal startingGold = decimal.Parse(PromptUser("How much gold would your character start with?"));
        ...
        public void DisplayItems(List<Item> retrieveAvailableItems)
        {
            retrieveAvailableItems.ForEach(item => Console.WriteLine($"{item.Id} - {item.Name} | {item.Cost:C}"));
        }
